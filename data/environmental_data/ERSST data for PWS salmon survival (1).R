...
# Open netCDF file
 nc <- open.ncdf("/Users/MikeLitzow/Documents/R/pdo/sst.mnmean.nc") # (on my computer)

# view dates
  d <- dates(get.var.ncdf(nc, "time"), origin=c(1,15,1800))
 d
...
# Extract GOA SST, 54-61 deg. N, 200-226 deg. E:
 ...
 x <- get.var.ncdf(nc, "lon", start=101, count=14)
 y <- get.var.ncdf(nc, "lat", start=14, count=5)
 x; y
 SST <- get.var.ncdf(nc, "sst", start=c(101,14,1153), count=c(14,5,length(d)), verbose = T)
