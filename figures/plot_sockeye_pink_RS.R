...
pdf("Sockeye_pink_RS.pdf")
par(mfrow = c(3,2), mai = c(0.4,0.4,0.2,0.03), mgp=c(2,1,0))
plot(copper$BroodYear[which(!is.na(copper$logRS))], residuals(copper.mod), main="Copper", xlab="",ylab="Residuals",type="b")
abline(c(0,0),col="red")
plot(copper$ad.hatchPinkRun.lag2[which(!is.na(copper$logRS))], residuals(copper.mod), main="Copper", xlab="Hatchery pink salmon returns",ylab="Residuals")
...
plot(eshamy$BroodYear[which(!is.na(eshamy$logRS))], residuals(eshamy.mod), main="Eshamy", xlab="",ylab="Residuals",type="b")
abline(c(0,0),col="red")
plot(eshamy$ad.hatchPinkRun.lag2[which(!is.na(eshamy$logRS))], residuals(eshamy.mod), main="Eshamy",xlab="Hatchery pink salmon returns",ylab="Residuals")
...
plot(coghill$BroodYear[which(!is.na(coghill$logRS))], residuals(coghill.mod), main="Coghill", xlab="",ylab="Residuals",type="b")
abline(c(0,0),col="red")
plot(coghill$ad.hatchPinkRun.lag2[which(!is.na(coghill$logRS))], residuals(coghill.mod), main="Coghill", xlab="Hatchery pink salmon returns",ylab="Residuals")
