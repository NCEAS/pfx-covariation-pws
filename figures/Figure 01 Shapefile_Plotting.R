...
stock_locs <- data.frame(lon=c(-147.814347, -144.88, -148.128593, -147.448752),
                         lat=c(61.09, 60.46, 60.451407, 61.079911))

#Location where Exxon hit Bligh Reef
EVOS <- data.frame(lon=-146.88, lat=60.84)
...
# I think the nepacLLhigh dataset was an unprojected WGS84 datum. If something is unprojected, you don't really need
# to use this next line most of the time I don't think. But I'm inluding it just to be thorough. 
...
# Now transform for ggplot world. Fortify converts a model or R object to a data
...
#Copper_Shape.sp <- readOGR(dsn="Shapefiles", layer = "copper_2m_lin") #OLD CODE FROM JORDAN
...
# Not very helpful without axes. 'axis() adds an axis to the current plot
axis(1);axis(2)


# It's in UTM. So we use the next line to convert to lat-long. 
# The nepacLLhigh dataset has datum WGS84. 
...
PWSMAP1 <- ggplot()+
  ...
  geom_path(data=myCopper.gg,aes(long,lat,group=factor(group)),color="black")+ # Experiment here. See what happens when you use geom_polygon vs. geom_line vs. geom_path
  ...
  geom_point(data=stock_locs, aes(lon,lat), size = 2.5, pch=17) +
  theme(axis.title.y=element_text(size=16),axis.title.x=element_text(size=16),
        axis.text.x=element_text(size=14),axis.text.y=element_text(size=14))+
  annotate("text", x = -147.1, y = 60.77, label = "EVOS") +
  annotate("text", x = -146, y = 61.2, label = "Valdez") +
  annotate("text", x = -145.4, y = 60.59, label = "Cordova") +
  annotate("text", x = -148.2, y = 61.19, label = "Coghill") +
  annotate("text", x = -145, y = 60.15, label = "Copper R.") +
  annotate("text", x = -148.5, y = 60.44, label = "Eshamy") +
  annotate("text", x = -147.2, y = 61.2, label = "Unakwik") 
  #annotate("segment", x=-147, xend=-146.91, y=60.73, yend=60.82,
           #arrow=arrow(ends="last", angle=20, length=unit(.2,"cm")))
PWSMAP1
